...
function generateDefaultPreview(value, varName) {
  // Detectar si es una referencia a otra variable
  const isVariableReference = /^var\(--[\w-]+\)/.test(value.trim());

  // Usar el icono apropiado según el tipo
  const iconSvg = isVariableReference
  ...
  return {
    ...
    tooltip: isVariableReference
      ? `Referencia a variable: ${value}`
      : `Valor no reconocido: ${value}`
  };
}
