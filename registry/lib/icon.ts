...
function createIconComponent(name: PictoName) {
  const IconComponent = React.forwardRef<SVGSVGElement, BaseIconProps>(
    ({ className, onClick, "aria-label": ariaLabel, ...props }, ref) => {
      ...
      return React.createElement(PictoComponent, {
        ref,
...
type CapitalizedIconNames = {
  [K in PictoName as Capitalize<K>]: React.ComponentType<BaseIconProps>
}

// Crear el objeto Icon con todas las propiedades de iconos
const IconComponents: CapitalizedIconNames = Object.keys(picto).reduce((acc, iconName) => {
  const capitalizedName = iconName.charAt(0).toUpperCase() + iconName.slice(1) as Capitalize<PictoName>
  acc[capitalizedName] = createIconComponent(iconName as PictoName)
  return acc
